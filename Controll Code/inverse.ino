...
byte x = 39, y = 15, z = 92, angle4 = 78, angle5 = 162, angle6 = 87;
...
void loop() {
    if (Serial.available()) {
        ...
        armSetPosition(x, y, z, angle4, angle5, 0);
        Serial.println(x);
        Serial.println(y);
        Serial.println(z);
...
void armSetAngle(byte angle1, byte angle2, byte angle3, byte angle4, byte angle5, byte angle6, byte speed) {
    ...
    servo5.write(angle6, speed);  //54 <=θ6<=123,default θ6=87
}
...
void gripperController(int mode) {
    if (mode == 0) {
    }
}